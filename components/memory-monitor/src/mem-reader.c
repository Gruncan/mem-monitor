...
static const char* mem_page_mapping[] = {
    "nr_free_pages",
...
void mem_parse_page_line(const char* line, MemVmInfo* mp) {
    char key[256];
    unsigned long value;
    ...
    char* new_line = strchr(key, '\n');
    if (new_line)
        *new_line = '\0';

    const size_t struct_length = sizeof(MemVmInfo);

    set_mem_struct_value(mp, struct_length, mem_page_mapping, key, value);
}

void mem_parse_line(const char* line, MemInfo* mem_info) {
    char key[256];
    unsigned long value;
    char unit[32];
    ...
    const size_t struct_length = sizeof(MemInfo);

    set_mem_struct_value(mem_info, struct_length, mem_mapping, key, value);
}
...
void read_mem_vm_info(MemVmInfo* mem_vm_info) {
    ...
    char* line = strtok(content, "\n");
    while (line != NULL) {
        mem_parse_page_line(line, mem_vm_info);
        line = strtok(NULL, "\n");
    }
