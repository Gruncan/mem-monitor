...
template <typename T> class AnimationDataContainer {
  public:
    explicit AnimationDataContainer(const uint32_t timeSpacing, const uint32_t stepSize)
        : timeSpacing(timeSpacing), head(timeSpacing), tail(0), stepSize(stepSize), data_all(nullptr),
          times_all(nullptr) {
        index = timeSpacing;
    ...
    uint32_t getTimeSpacing() const { return this->timeSpacing; }

    void setTimes(QVector<double>* times) { this->times_all = times; }

    void setData(QVector<double>* data) { this->data_all = data; }
    ...
    void rewind() { tail -= (2 * stepSize); }

    void forward() { tail += stepSize; }
